...
export default async function handler(
	...
	const { salon_id, staff_id, service_id, datetime } = JSON.parse(req.body)
	...
	const URL = `${PROD_URL}/book_times/${salon_id}?${staff_id ? `&staff_id=${staff_id}` : ''}${service_id ? `&service_id=${service_id}` : ''}${datetime ? `&datetime=${datetime}` : ''}`
